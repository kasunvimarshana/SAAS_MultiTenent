...
class WebhookService
{
    public function triggerWebhook(int|string $tenantId, string $event, array $data): void
    {
        try {
            ...
            if (!$tenant || empty($tenant->webhook_url)) { return; }
            ...
            Http::withHeaders(['X-Webhook-Signature' => $signature, 'X-Webhook-Event' => $event, 'X-Tenant-ID' => (string) $tenantId])
                ->timeout(10)->post($tenant->webhook_url, $payload->toArray());
        } catch (\Throwable $e) {
            Log::error('WebhookService: Failed', ['error' => $e->getMessage()]);
        }
    }
}
