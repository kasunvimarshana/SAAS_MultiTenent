...
abstract class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function search(string $query, array $searchable = [], array $columns = ['*']): Collection
    {
        ...
        $builder = $this->query;
        $builder->where(function (Builder $q) use ($escaped, $searchable) {
        ...
        foreach ($filters as $field => $value) {
            if (is_array($value)) {
                if (isset($value['operator']) && isset($value['value'])) {
                    ...
                    $builder->whereBetween($field, $value['between']);
    ...
    /**
     * Conditionally paginate: returns paginated results if per_page exists, all results otherwise.
     * Works with queries, arrays, collections, API responses, and other iterable data sources.
     */
    public function conditionalPaginate(array $params, array $columns = ['*']): LengthAwarePaginator|Collection
    {
        if (isset($params['search']) && !empty($params['search'])) {
            ...
            $this->search($params['search'], $searchable, $columns);
            // Re-apply since search resets query
            $builder = $this->model->newQuery();
            if (!empty($this->with)) {
                $builder->with($this->with);
            }
            $escapedSearch = str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $params['search']);
            $builder->where(function (Builder $q) use ($escapedSearch, $searchable) {
                foreach ($searchable as $field) {
        ...
        if (isset($params['sort_by'])) {
            ...
            $this->query->orderBy($params['sort_by'], $direction);
        }
    ...
    /**
     * Paginate any iterable (array, Collection, etc.) conditionally.
     */
    public function paginateIterable(iterable $data, array $params): LengthAwarePaginator|Collection
    {
        ...
        if (isset($params['search']) && !empty($params['search'])) {
            ...
            $needle = strtolower($params['search']);
            $collection = $collection->filter(function ($item) use ($needle, $searchable) {
                foreach ($searchable as $field) {
        ...
        if (isset($params['sort_by'])) {
            ...
            $collection = $direction === 'desc'
                ? $collection->sortByDesc($field)->values()
                : $collection->sortBy($field)->values();
        }
